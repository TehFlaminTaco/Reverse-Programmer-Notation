...
<head>
	<script>
		...
		function dorpn(){
			...
			rpn_code = rpn_code.replace(/<CODETORUN>/gi, '[['+rpn+']]');
			var i = inp.replace(/(\s|^)+(("(.*?)")|(\S+))/gi, ', [[$5$4]]')
			rpn_code = rpn_code.replace(/<INPUTS>/gi, i)
...
<body>
	<textarea name="CodeInput" id="CodeInput"></textarea>
	<form>
	...
	<div id="footer">
		Credit to <a href='https://daurnimator.github.io/lua.vm.js/lua.vm.js.html'>lua.vm.js</a> for their Lua VM. Which actively interprets Reverse Programmer Notation.
	</div>
